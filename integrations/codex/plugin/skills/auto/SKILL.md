...
- **Known path → straight to `lc.read`; any repo search → `lc.code_search`** — symbols, strings, regex, every term in one query. Inline source is already read.
- **`lc.bash` = execution only.** Never `grep -r`/`rg`/`git grep`/`find`/`sed`/`cat`/`head` over the repo; shell search only logs, scratch, command output.
